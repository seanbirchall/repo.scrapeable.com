...
#' U.S. Drought Monitor. Values represent the percentage of the state's land
#' area in each drought category. Categories are cumulative:
...
#'   \code{"TX"}, \code{"NY"}) or two-digit FIPS code (e.g., \code{"06"}).
#' @param start Character. Start date in \code{"M/D/YYYY"} format
#'   (e.g., \code{"1/1/2024"}). Default is \code{"1/1/2024"}.
#' @param end Character. End date in \code{"M/D/YYYY"} format
#'   (e.g., \code{"12/31/2024"}). Default is \code{"12/31/2024"}.
#' @return A tibble with columns:
#'   \describe{
#'     \item{date}{Date. Weekly report date (Tuesdays).}
#'     \item{state}{Character. Two-letter state abbreviation.}
#'     \item{none}{Numeric. Percent of area with no drought.}
#'     \item{d0}{Numeric. Percent of area at D0 (Abnormally Dry) or worse.}
#'     \item{d1}{Numeric. Percent of area at D1 (Moderate Drought) or worse.}
#'     \item{d2}{Numeric. Percent of area at D2 (Severe Drought) or worse.}
#'     \item{d3}{Numeric. Percent of area at D3 (Extreme Drought) or worse.}
#'     \item{d4}{Numeric. Percent of area at D4 (Exceptional Drought).}
#'   }
#' @examples
#' drought_state("CA", start = "1/1/2024", end = "6/30/2024")
#' drought_state("TX", start = "1/1/2023", end = "12/31/2023")
...
#'
#' Returns weekly drought severity percentages for all counties in a given
#' state. Same drought categories as \code{drought_state()} but at county
#' resolution.
#'
#' @param state Character. Two-letter state abbreviation (e.g., \code{"CA"},
#'   \code{"TX"}) or two-digit FIPS code.
#' @param start Character. Start date in \code{"M/D/YYYY"} format.
#'   Default is \code{"1/1/2024"}.
#' @param end Character. End date in \code{"M/D/YYYY"} format.
#'   Default is \code{"12/31/2024"}.
#' @return A tibble with columns:
#'   \describe{
#'     \item{date}{Date. Weekly report date (Tuesdays).}
#'     \item{county}{Character. County name or FIPS code.}
#'     \item{state}{Character. Two-letter state abbreviation.}
#'     \item{none}{Numeric. Percent of county area with no drought.}
#'     \item{d0}{Numeric. Percent at D0 (Abnormally Dry) or worse.}
#'     \item{d1}{Numeric. Percent at D1 (Moderate Drought) or worse.}
#'     \item{d2}{Numeric. Percent at D2 (Severe Drought) or worse.}
#'     \item{d3}{Numeric. Percent at D3 (Extreme Drought) or worse.}
#'     \item{d4}{Numeric. Percent at D4 (Exceptional Drought).}
#'   }
#' @examples
#' drought_county("CA", start = "1/1/2024", end = "3/31/2024")
...
#'
#' Fetches the most recent week's drought severity percentages for all
#' 50 U.S. states. Makes one API call per state, so execution takes
#' approximately 30-60 seconds.
#'
#' @return A tibble with one row per state and columns:
#'   \describe{
#'     \item{date}{Date. Most recent weekly report date.}
#'     \item{state}{Character. Two-letter state abbreviation.}
#'     \item{none}{Numeric. Percent of area with no drought.}
#'     \item{d0}{Numeric. Percent at D0 or worse.}
#'     \item{d1}{Numeric. Percent at D1 or worse.}
#'     \item{d2}{Numeric. Percent at D2 or worse.}
#'     \item{d3}{Numeric. Percent at D3 or worse.}
#'     \item{d4}{Numeric. Percent at D4 (Exceptional Drought).}
#'   }
#' @examples
#' drought_latest()
...
#'
#' Fetches drought data for multiple states and returns them in a single
#' tibble for easy comparison. Makes one API call per state.
#'
#' @param states Character vector. State abbreviations to compare
#'   (e.g., \code{c("CA", "TX", "AZ")}).
#' @param start Character. Start date in \code{"M/D/YYYY"} format.
#'   Default is \code{"1/1/2024"}.
#' @param end Character. End date in \code{"M/D/YYYY"} format.
...
#' @return A tibble with columns:
#'   \describe{
#'     \item{date}{Date. Weekly report date.}
#'     \item{state}{Character. Two-letter state abbreviation.}
#'     \item{none}{Numeric. Percent with no drought.}
#'     \item{d0}{Numeric. Percent at D0 or worse.}
#'     \item{d1}{Numeric. Percent at D1 or worse.}
#'     \item{d2}{Numeric. Percent at D2 or worse.}
#'     \item{d3}{Numeric. Percent at D3 or worse.}
...
#'   }
#' @examples
#' drought_compare(c("CA", "TX", "AZ"), start = "1/1/2024", end = "6/30/2024")
...
#' Get droughtmonitor.unl.edu client context for LLM use
#'
...
drought_context <- function() {
  ...
  if (is.null(src_file) || !file.exists(src_file)) src_file <- "clients/droughtmonitor.unl.edu.R"
  if (!file.exists(src_file)) {
    pkg_src <- system.file("source", package = "droughtmonitor.unl.edu")
    if (nzchar(pkg_src)) {
      sf <- list.files(pkg_src, pattern = "[.]R$", full.names = TRUE)
      if (length(sf)) src_file <- sf[1]
    }
  }
  if (!file.exists(src_file)) { cat("# droughtmonitor.unl.edu context - source not found\n"); return(invisible("")) }
  ...
  out <- paste(c("# droughtmonitor.unl.edu", "# R API Client", "#", "# == Public Functions ==", "#", unlist(blocks)), collapse = "\n")
  cat(out, "\n"); invisible(out)
}
