...
#' Get Wake County development records
#'
#' Fetches subdivision development planning records from the Wake County
#' ArcGIS FeatureServer. Pagination is handled automatically.
#'
#' @param status Optional plan status to filter by (e.g. "Approved",
#'   "Approval Expired", "Preliminary"). Case-insensitive partial match.
#' @return tibble with columns: object_id, case_id, case_number,
#'   work_class, application_date, pin_number, description, plan_status,
#'   subdivision_name, proposed_lots, acres
#' @export
...
#' Summarize Wake County developments by status
#'
#' Returns counts of development records grouped by plan status.
#'
#' @return tibble with columns: plan_status, n_records, total_lots, total_acres
#' @export
...
#'
#' Case-insensitive search across subdivision name and description fields.
#'
#' @param query Character string to search for
#' @return tibble (same schema as wake_developments)
#' @export
...
#' Return context about the Wake County client functions
#'
#' @return Character string of function signatures and documentation
#' @export
wake_context <- function() {
  .build_context(
    pkg_name = "wakegov.com",
    header_lines = c(
      "# Wake County Development Data Client",
      "# Source: ArcGIS FeatureServer",
      "# Base URL: https://maps.wakegov.com/arcgis/rest/services/Planning/Wake_County_Development/FeatureServer/0"
    )
  )
}
