...
#'
#' Queries the DKAN metastore for published dataset metadata on the
#' federal healthcare open data portal. Returns all datasets across
#' all pages when paginated.
#'
#' @param page_size Integer. Number of results per page (default 20).
#'   Note: setting a large value (e.g. 500) returns all datasets at once.
#' @param page Integer. Page number, 1-based (default 1).
#' @return A tibble with columns: \code{identifier} (character, dataset slug
#'   e.g. \code{"e4rr-zk4i"}), \code{title} (character), \code{description}
#'   (character), \code{modified} (character, ISO date), \code{access_level}
#'   (character, e.g. \code{"public"}), \code{keyword} (character,
#'   comma-separated tags).
#' @examples
#' \dontrun{
#' hcgov_datasets()
#' hcgov_datasets(page_size = 5, page = 2)
#' }
#' @export
...
#'
#' Retrieves full metadata for a single dataset from data.healthcare.gov
#' including publisher, contact, and distribution information.
#'
#' @param id Character. Dataset identifier from \code{\link{hcgov_datasets}}
#'   (e.g. \code{"e4rr-zk4i"}, \code{"wb6u-x2ny"}).
#' @return A tibble with one row and columns: \code{identifier}, \code{title},
#'   \code{description}, \code{modified}, \code{access_level},
#'   \code{publisher_name} (character), \code{contact_name} (character),
#'   \code{contact_email} (character), \code{distribution_count} (integer,
#'   number of downloadable files/resources).
#' @examples
#' \dontrun{
...
#' Get healthcare.gov client context for LLM use
#'
...
hcgov_context <- function() {
  ...
  if (is.null(src_file) || !file.exists(src_file)) src_file <- "clients/healthcare.gov.R"
  if (is.null(src_file) || !file.exists(src_file)) {
    pkg_src <- system.file("source", package = "healthcare.gov")
    if (nzchar(pkg_src)) {
      sf <- list.files(pkg_src, pattern = "[.]R$", full.names = TRUE)
      if (length(sf)) src_file <- sf[1]
    }
  }
  if (!file.exists(src_file)) { cat("# healthcare.gov context - source not found\n"); return(invisible("")) }
  ...
  out <- paste(c("# healthcare.gov", "# R API Client", "#", "# == Public Functions ==", "#", unlist(blocks)), collapse = "\n")
  cat(out, "\n"); invisible(out)
}
