...
#' Get finance-yahoo client context for LLM use
#'
...
yf_context <- function() {
  ...
  if (is.null(src_file) || !file.exists(src_file)) src_file <- "clients/finance-yahoo.R"
  if (!file.exists(src_file)) {
    pkg_src <- system.file("source", package = "finance-yahoo")
    if (nzchar(pkg_src)) {
      sf <- list.files(pkg_src, pattern = "[.]R$", full.names = TRUE)
      if (length(sf)) src_file <- sf[1]
    }
  }
  if (!file.exists(src_file)) { cat("# finance-yahoo context - source not found\n"); return(invisible("")) }
  ...
  out <- paste(c("# finance-yahoo", "# R API Client", "#", "# == Public Functions ==", "#", unlist(blocks)), collapse = "\n")
  cat(out, "\n"); invisible(out)
}
