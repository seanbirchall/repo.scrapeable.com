...
#' Generate LLM-friendly context for waterdata.usgs.gov
#'
...
water_context <- function() {
  ...
  if (is.null(src_file)) src_file <- "clients/waterdata.usgs.gov.R"
  if (!file.exists(src_file)) {
    cat("# waterdata.usgs.gov context - source not found\n")
    return(invisible("# waterdata.usgs.gov context - source not found"))
  }
