...
sec_filings <- function(cik, form = NULL, all = FALSE) {
  ...
  if (!is.null(form)) df <- df |> filter(toupper(form) == toupper(!!form))
  df |>
...
sec_insiders_bulk <- function(year = NULL, quarter = NULL) {
  ...
  results <- lapply(links$url, function(url) {
    tryCatch(.parse_insiders_zip(.fetch(url, ".zip")),
...
sec_13fs_bulk <- function(n = NULL, year = NULL) {
  ...
  results <- lapply(links$url, function(url) {
    tryCatch(.parse_13f_zip(.fetch(url, ".zip")),
             error = function(e) { message("Failed: ", url, " - ", e$message); NULL })
  })
  bind_rows(results)
}
...
sec_ftds_bulk <- function(n = NULL, year = NULL, month = NULL) {
  ...
  results <- lapply(links$url, function(url) {
    tryCatch(.parse_ftd_zip(.fetch(url, ".zip")),
             error = function(e) { message("Failed: ", url, " - ", e$message); NULL })
  })
  bind_rows(results)
}
...
print.sec_filing <- function(x, ...) {
  ...
  for (cat_name in cat_order) {
    ...
    if (cat_name == "primary") {
      ...
      stmts <- stmts[grepl("CONSOLIDATED|BALANCE|INCOME|CASH|OPERATIONS|EQUITY|STATEMENT",
                            stmts, ignore.case = TRUE)]
...
sec_filing_xbrl_holding <- function(x) {
  ...
  tibble(
    ...
    voting_sole    = xml2::xml_text(xml2::xml_find_first(entries, ".//*[local-name()='Sole']")),
    voting_shared  = xml2::xml_text(xml2::xml_find_first(entries, ".//*[local-name()='Shared']")),
    voting_none    = xml2::xml_text(xml2::xml_find_first(entries, ".//*[local-name()='None']")),
    accession      = accession
