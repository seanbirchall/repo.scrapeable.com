...
#'
#' Downloads and parses the official AWS IP ranges JSON file, returning
#' all IPv4 CIDR prefixes. Useful for firewall rules, network security
#' analysis, and cloud infrastructure auditing.
#'
#' @param service Filter by AWS service name (e.g. "S3", "EC2", "CLOUDFRONT",
#'   "AMAZON", "ROUTE53"). Default NULL returns all services.
#' @param region Filter by region pattern (e.g. "us-east-1", "eu-").
#'   Uses partial matching via grepl. Default NULL returns all regions.
#' @return A tibble with columns:
#'   \describe{
#'     \item{ip_prefix}{IPv4 CIDR block, e.g. "54.231.0.0/16" (character)}
#'     \item{region}{AWS region, e.g. "us-east-1" (character)}
#'     \item{service}{AWS service name (character)}
#'     \item{network_border_group}{Network border group (character)}
#'   }
#' @examples
#' awsip_ranges(service = "S3")
#' awsip_ranges(region = "us-east-1")
#' @seealso [awsip_ipv6_ranges()], [awsip_services()], [awsip_regions()], [awsip_context()]
...
#'
#' Downloads and parses the official AWS IP ranges JSON file, returning
#' all IPv6 CIDR prefixes with optional filtering.
#'
#' @param service Filter by AWS service name. Default NULL returns all.
#' @param region Filter by region pattern (partial match). Default NULL.
#' @return A tibble with columns:
#'   \describe{
#'     \item{ipv6_prefix}{IPv6 CIDR block (character)}
#'     \item{region}{AWS region (character)}
#'     \item{service}{AWS service name (character)}
#'     \item{network_border_group}{Network border group (character)}
#'   }
#' @examples
#' awsip_ipv6_ranges(service = "EC2")
#' @seealso [awsip_ranges()], [awsip_services()], [awsip_context()]
#' @source <https://ip-ranges.amazonaws.com/ip-ranges.json>
#' @export
...
#'
#' Summarizes the AWS IP ranges by service, showing the number of
#' IPv4 and IPv6 prefixes allocated to each service.
#'
#' @return A tibble with columns:
#'   \describe{
#'     \item{service}{AWS service name (character)}
...
#' @examples
#' awsip_services()
#' @seealso [awsip_ranges()], [awsip_regions()], [awsip_context()]
#' @export
...
#'
#' Summarizes the AWS IP ranges by region, showing the number of
#' IPv4 and IPv6 prefixes allocated to each AWS region.
#'
#' @return A tibble with columns:
#'   \describe{
#'     \item{region}{AWS region name, e.g. "us-east-1" (character)}
#'     \item{ipv4_count}{Number of IPv4 CIDR blocks (integer)}
#'     \item{ipv6_count}{Number of IPv6 CIDR blocks (integer)}
#'   }
#' @examples
